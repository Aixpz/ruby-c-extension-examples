...
**错误信息 4a**：
```
error : 元素 <LanguageStandard> 具有无效值"stdcpp11"
```

**解决方案**：
项目已修复为使用 `stdcpp14`。如果仍有问题，可以：
1. 项目属性 → C/C++ → 语言 → C++ 语言标准 → 选择 "ISO C++14" 或 "ISO C++17"
2. 或完全移除 LanguageStandard 设置，使用编译器默认值
...
**解决方案**：
1. 确认项目设置了 C++ 标准
2. 项目属性 → C/C++ → 语言 → C++ 语言标准 → 选择 "ISO C++14" 或更高
